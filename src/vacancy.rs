...
impl From<status::Status> for Vacancy {
    fn from(status: status::Status) -> Self {
        Vacancy {
            ...
            uri: status.uri,
            url: status.url,
            account: Some(Account::from(&status.account)),
            content: Some(status.content),
            created_at: Some(status.created_at),
            created_at_ts: Some(status.created_at.timestamp_millis()),
            media_attachments: Some(Attachment::from(status.media_attachments)),
            tags: Some(Tag::from(status.tags)),
